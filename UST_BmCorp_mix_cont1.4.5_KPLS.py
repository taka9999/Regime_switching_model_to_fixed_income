...
datadict = 'data/'
asset1 = 'UST10'
asset1file = 'TRXVUSGOV10U(TOT_RETURN)_import'
asset2 = 'BmCorp'
asset2file = 'luactruu'
...
## Data Preparation
df = pd.read_csv(datadict + asset1file + '.csv',index_col = 0)
df = df.reset_index()
...
df2 = pd.read_csv(datadict + asset2file + '.csv',index_col = 0,skiprows=5)
df2 = df2.reset_index()
df2 = df2.rename(columns={"Date": "date"})
df2['date'] = pd.to_datetime(df2['date'],format = '%m/%d/%Y')
df2 = df2.sort_values("date", ascending=True)
df2["ret"] = np.log(df2["PX_LAST"]).diff() # log returns
data2 = df2[["date", "ret"]].dropna()

data = pd.merge(data1, data2, on="date", suffixes=('_' + asset1, '_' + asset2), how='inner')
data["diffret"] = data["ret_" + asset1] - data["ret_" + asset2]
data = data[["date", "diffret"]]


ret_ser = pd.Series(
    data["diffret"].values,
    index=data["date"],
    name="diffret of " + asset1 + " and " + asset2
)
ret_ser1 =pd.Series(
    data1["ret"].values,
    index=data1["date"],
...
pd.to_pickle(data_obj, datadict + "diffret of " + asset1 + " and " + asset2 + ".pkl")
class DataObj:
    pass
# daily data is from 1980-01-01
dl = DataLoader(ticker="diffret of " + asset1 + " and " + asset2, ver="v0").load(start_date=datastart, end_date=dataend)
...
for pname, (umodel, ext_features_df) in patterns.items():
    ...
    while t0 + relativedelta(months=pred_months) <= val_end:
        ...
        xgb_full = XGBClassifier(objective='binary:logistic',eval_metric='logloss',random_state=0)
    ...
    perf_bh2 = compute_performance(
        ret_series = ret2_aligned.reindex(pred_idx).fillna(0.0),
        rf_series  = rf_aligned,
        freq       = 252,
        benchmark  = bench.reindex(pred_idx).fillna(0.0)
    )
...
perf_df = perf_df.applymap(lambda cell: cell.item() if isinstance(cell, pd.Series) and cell.size == 1 else cell)


import pickle
resdir = 'results'
nbname = 'UST_BmCorp_mix_cont1.4.5_GPU'
