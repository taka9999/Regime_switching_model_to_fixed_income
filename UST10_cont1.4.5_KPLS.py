...
for pname, (umodel, ext_features_df) in patterns.items():
    ...
    while t0 + relativedelta(months=pred_months) <= val_end:
        ...
        for lam in lambda_list:
            for alpha in reg_alpha_list:
                ...
                for train_idx, val_idx in tscv.split(Xj_train):
                    ...
                    # 3) ロジスティック回帰で学習
                    xgb = XGBClassifier(objective='binary:logistic',eval_metric='logloss',random_state=0, base_score = 0.5)
        ...
        xgb_full = XGBClassifier(objective='binary:logistic',eval_metric='logloss',random_state=0, base_score = 0.5)
...
import pickle
resdir = 'results'
nbname = 'UST10_cont1.4.5_GPU_rqPLS_v2'
