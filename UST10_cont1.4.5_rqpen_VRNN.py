...
class SlidingWindowDataset(Dataset):
    """
    X: (T, D) 2D ndarray
    y: (T,)   0/1 ラベル（または確率→しきい値で0/1にしておく）
    idx: pandas.DatetimeIndex 等（長さT）。返すのはウィンドウの末尾の index。
    L: ウィンドウ長
    stride: ステップ幅
    """
    def __init__(self, X, y, idx, L=32, stride=1):
        ...
        self.stride = stride
        self._starts = []
        for s in range(0, len(X) - L + 1, stride):
    ...
    def __getitem__(self, i):
        s = self._starts[i]
        e = s + self.L
        x_win = self.X[s:e]             # (L, D)
        y_last = self.y[e-1]            # 末尾の時点ラベル
        t_last = self.idx[e-1]          # 末尾の時点インデックス
        return torch.from_numpy(x_win), torch.tensor(y_last), str(t_last)
import torch
import torch.nn as nn
import torch.nn.functional as F

class VRNNLite(nn.Module):
    def __init__(self, x_dim, z_dim, h_dim=64):
        super().__init__()
        self.phi_x   = nn.Sequential(nn.Linear(x_dim, h_dim), nn.ReLU())
        self.phi_z   = nn.Sequential(nn.Linear(z_dim, h_dim), nn.ReLU())
        self.enc_mu     = nn.Linear(h_dim + h_dim, z_dim)
        self.enc_logvar = nn.Linear(h_dim + h_dim, z_dim)
        self.dec_x = nn.Sequential(nn.Linear(h_dim + h_dim, x_dim))
        self.rnn = nn.LSTMCell(input_size=h_dim + h_dim, hidden_size=h_dim)
        self.cls = nn.Sequential(nn.Linear(h_dim, 64), nn.ReLU(), nn.Linear(64, 2))  # 2クラス
    ...
    def forward(self, x):
        ...
        B, L, D = x.size()
        device = x.device
        h = torch.zeros(B, self.rnn.hidden_size, device=device)
        c = torch.zeros(B, self.rnn.hidden_size, device=device)
        ...
        for t in range(L):
            xt = x[:, t, :]                       # (B,D)
            phi_x_t = self.phi_x(xt)              # (B,h)

            enc_in  = torch.cat([phi_x_t, h], dim=-1)
            mu_t    = self.enc_mu(enc_in)         # (B,z)
            lv_t    = self.enc_logvar(enc_in)     # (B,z)
            ...
            dec_in  = torch.cat([phi_z_t, h], dim=-1)
            x_hat_t = self.dec_x(dec_in)          # (B,D)
            recons.append(x_hat_t)

            rnn_in  = torch.cat([phi_x_t, phi_z_t], dim=-1)
            h, c    = self.rnn(rnn_in, (h, c))

            logits_t.append(self.cls(h))          # (B,2)

            # KL(N(mu_t, σ^2) || N(0,1))
            kld = -0.5 * (1 + lv_t - mu_t.pow(2) - lv_t.exp()).mean()
            kld_loss = kld_loss + kld

        x_hat = torch.stack(recons, dim=1)        # (B,L,D)
        z_seq = torch.stack(zs, dim=1)            # (B,L,z)
        logit_seq = torch.stack(logits_t, dim=1)  # (B,L,2)
        logit_last = logit_seq[:, -1, :]          # (B,2)
        return x_hat, z_seq, kld_loss / L, logit_seq, logit_last
...
def gaussian_kl(mu_q, lv_q, mu_p, lv_p):
    # 全て (B, z) で想定。lv = log-variance
    # D_KL( N(mu_q, diag(exp(lv_q))) || N(mu_p, diag(exp(lv_p))) )
    var_q = lv_q.exp()
    var_p = lv_p.exp()
    term = (var_q / var_p) + (mu_p - mu_q).pow(2) / var_p - 1 + (lv_p - lv_q)
    return 0.5 * term.sum(dim=-1)  # (B,)

class VRNN_Prior(nn.Module):
    """
    VRNN with learned prior:
      q(z_t | x_t, h_{t-1}) = N(mu_q, Sigma_q)
      p(z_t | h_{t-1}, z_{t-1}?) = N(mu_p, Sigma_p)
      p(x_t | z_t, h_{t-1})
    2クラス分類ヘッド付き（末尾h_tから）。
    """
    def __init__(self, x_dim, z_dim, h_dim=128, ar_prior=False):
        super().__init__()
        self.z_dim = z_dim
        self.h_dim = h_dim
        ...
        # inference network q(z|x,h)
        self.enc = nn.Sequential(nn.Linear(h_dim + h_dim, h_dim), nn.ReLU())
        self.enc_mu = nn.Linear(h_dim, z_dim)
        self.enc_lv = nn.Linear(h_dim, z_dim)

        # learned prior p(z|h[, z_{t-1}])
        prior_in_dim = h_dim + (z_dim if ar_prior else 0)
        self.prior = nn.Sequential(nn.Linear(prior_in_dim, h_dim), nn.ReLU())
        self.prior_mu = nn.Linear(h_dim, z_dim)
        self.prior_lv = nn.Linear(h_dim, z_dim)  # logvar

        # decoder p(x|z,h)
        self.dec = nn.Sequential(nn.Linear(h_dim + h_dim, h_dim), nn.ReLU())
        self.dec_x = nn.Linear(h_dim, x_dim)

        # recurrent core
        self.rnn = nn.LSTMCell(input_size=h_dim + h_dim, hidden_size=h_dim)

        # classifier on h_t (use last)
        self.cls = nn.Sequential(nn.Linear(h_dim, 64), nn.ReLU(), nn.Linear(64, 2))

        # optional: init prior logvar biases to small negative for stable start
        nn.init.constant_(self.prior_lv.bias, -2.0)

    def forward(self, x, beta_kl=1.0):
        # x: (B,L,D) or (B,D)
        if x.dim() == 2:
            x = x.unsqueeze(1)
        ...
        c = torch.zeros(B, self.h_dim, device=device)

        z_prev = torch.zeros(B, self.z_dim, device=device)

        recons, z_list, logits_t = [], [], []
        kls = []
        ...
        for t in range(L):
            ...
            # --- prior p(z_t | h_{t-1}[, z_{t-1}]) ---
            if self.ar_prior:
                prior_in = torch.cat([h, z_prev], dim=-1)
            else:
                prior_in = h
            hp = self.prior(prior_in)
            mu_p = self.prior_mu(hp)
            lv_p = self.prior_lv(hp)      # logvar

            # --- inference q(z_t | x_t, h_{t-1}) ---
            enc_in = torch.cat([phi_x_t, h], dim=-1)
            he = self.enc(enc_in)
            mu_q = self.enc_mu(he)
            lv_q = self.enc_lv(he)        # logvar

            # reparam sample
            std_q = torch.exp(0.5 * lv_q)
            eps   = torch.randn_like(std_q)
            z_t   = mu_q + eps * std_q    # sample
            z_list.append(z_t)

            # --- reconstruction p(x_t | z_t, h_{t-1}) ---
            phi_z_t = self.phi_z(z_t)
            hd = self.dec(torch.cat([phi_z_t, h], dim=-1))
            x_hat_t = self.dec_x(hd)
            recons.append(x_hat_t)

            # RNN update with both x,z features
            rnn_in = torch.cat([phi_x_t, phi_z_t], dim=-1)
            h, c = self.rnn(rnn_in, (h, c))
            logits_t.append(self.cls(h))

            # KL(q||p) at time t
            kl_t = gaussian_kl(mu_q, lv_q, mu_p, lv_p)  # (B,)
            kls.append(kl_t)

            z_prev = z_t

        x_hat = torch.stack(recons, dim=1)         # (B,L,D)
        z_seq = torch.stack(z_list, dim=1)         # (B,L,z)
        logit_seq = torch.stack(logits_t, dim=1)   # (B,L,2)
        logit_last = logit_seq[:, -1, :]           # (B,2)
        kl_mean = torch.stack(kls, dim=1).mean()   # scalar mean over B,L

        # beta_kl でKLをアニーリング（0→1へスケジュール）
        return x_hat, z_seq, beta_kl * kl_mean, logit_seq, logit_last
...
def evaluate_params(lam, n_comp, Xj_train, ret_all, rf_all, ext_features_df, jm_setting, tr_cost):
    ...
    for train_idx, val_idx in tscv.split(Xj_train):
        ...
        ds_tr = SlidingWindowDataset(X_tr_mat, y_tr_bin, X_feat_tr.index, L=L, stride=stride)
        loader = DataLoader(ds_tr, batch_size=batch_size, shuffle=True)

        model  = VRNN_Prior(x_dim=X_tr_mat.shape[1], z_dim=n_comp, h_dim=64).to(device)
        opt    = torch.optim.Adam(model.parameters(), lr=1e-3)
        recon_loss = nn.MSELoss(reduction='mean')   # 系列なら mean で十分
        ...
        for epoch in range(70):
            ...
            for xb, yb, _ in loader:
                xb, yb = xb.to(device), yb.to(device)      # xb: (B,L,D)
                x_hat, z_seq, kld, logit_seq, logit_last = model(xb)

                # --- 損失 ---
                L_recon = recon_loss(x_hat, xb)            # (B,L,D) vs (B,L,D)
                L_ce    = ce_loss(logit_last, yb)          # 末尾時点の分類

                loss = L_recon + kld + 50.0 * L_ce
                ...
                if z_seq.size(1) >= 2:
                    ...
                    # 連続時点の確率KL
                    p = torch.softmax(logit_seq, dim=-1)[:, :, 1]  # (B,L)
                    p_t, p_tm1 = p[:, 1:], p[:, :-1]
                    eps = 1e-6
                    p_t   = p_t.clamp(eps, 1 - eps)
                    p_tm1 = p_tm1.clamp(eps, 1 - eps)
                    kl_ts = p_t * (p_t / p_tm1).log() + (1 - p_t) * ((1 - p_t) / (1 - p_tm1)).log()
                    loss = loss + 10.0 * kl_ts.mean()
            ...
            #print(f"[Epoch {epoch}] loss={total/nobs:.4f}")


        X_feat_val = pd.concat([X_feat_tr[-L:], X_feat_val], axis=0)
        X_val_mat = scaler.transform(X_feat_val.values)       # (m_samples, n_feats)
        # dummy y_val
        y_val = np.zeros(len(X_val_mat), dtype=int)
        y_val_bin = pd.Series(y_val).values
        ds_val = SlidingWindowDataset(X_val_mat, y_val_bin, X_feat_val.index, L=L, stride=1)
        ...
        all_tidx = []
        with torch.no_grad():
            for xb, yb, t_last in loader_val:
                xb = xb.to(device)
                _, _, _, _, logit_last = model(xb)
                prob1 = torch.softmax(logit_last, dim=-1)[:, 1].cpu().numpy()
                all_prob.append(prob1)
                all_tidx.extend(list(t_last))
        prob_val = np.concatenate(all_prob)  # (N_windows,)
        idx_val_last = pd.to_datetime(all_tidx)

        # 時系列へ
        proba_series = pd.Series(prob_val, index=idx_val_last).sort_index()

        # 平滑＆しきい値
        smooth = proba_series#.ewm(halflife=4, adjust=False).mean()
...
for pname, (umodel, ext_features_df) in patterns.items():
    ...
    while t0 + relativedelta(months=pred_months) <= val_end:
        ...
        proba_full = jm_final.proba_
        y_train_full = pd.Series(proba_full.iloc[1:,1].values,index=Xj_train.index[:-1])
        y_train_full_bin = (y_train_full > 0.5).astype(int)
        y_tr_full_bin   = y_train_full_bin.values
        
        X_feat_tr_full = pd.concat([Xj_train.loc[Xj_train.index[:-1]],ext_features_df.reindex(Xj_train.index[:-1])], axis=1)
        X_feat_tr_full = X_feat_tr_full.dropna(axis=1, how='any')
    
        feature_cols = X_feat_tr_full.columns
    
        scaler_full = StandardScaler().fit(X_feat_tr_full.values)
        X_tr_mat_full = scaler_full.transform(X_feat_tr_full.values)
        ...
        # 必要なら dropna(axis=0) で行を落とす
        
    
        ds_tr = SlidingWindowDataset(X_tr_mat_full, y_tr_full_bin, X_feat_tr_full.index, L=L, stride=stride)
        loader = DataLoader(ds_tr, batch_size=batch_size, shuffle=True)
    
        model  = VRNN_Prior(x_dim=X_tr_mat_full.shape[1], z_dim=best_n_comp, h_dim=64).to(device)
        opt    = torch.optim.Adam(model.parameters(), lr=1e-3)
        ...
        X_feat_val_full = pd.concat([X_feat_tr_full[-L:], X_feat_val_full], axis=0)
        X_val_mat_full = scaler_full.transform(X_feat_val_full.values)
        y_val = np.zeros(len(X_val_mat_full), dtype=int)
        y_val_bin = pd.Series(y_val).values
        ds_val = SlidingWindowDataset(X_val_mat_full, y_val_bin, X_feat_val_full.index, L=L, stride=1)
        loader_val = DataLoader(ds_val, batch_size=batch_size, shuffle=False)
        ...
        pred_idx = X_feat_val_full.index[L+1:]
        y_pred    = pd.Series(y_pred[1:-1].values, index=pred_idx)  # 翌日シフト
        ...
        clf_report = classification_report(jm_true.loc[pred_idx],y_pred.loc[jm_true[1:].index],target_names=["growth","crash"],labels = [0,1],zero_division=0)
...
import pickle
resdir = 'results'
nbname = 'UST10_cont1.4.5_GPU_rqpen_VRNN'
