...
from jumpmodels.jump_kernel_spect_dpen_v2 import JumpModelGPU as JumpModel  # class of Jump Model
...
jm_setting = dict(
    ...
    use_spectral_penalty = True, sp_kernel = 'rq', sp_kernel_params = {'alpha': 20, 'length_scale':10}, normalize_laplacian = True,
    #use_weighted_jump_penalty = True, wjp_kernel = 'rq', wjp_kernel_params = {'alpha': 20, 'length_scale':100}, wjp_normalize_laplacian = True,
    )
...
lambda_list = np.logspace(2,4,6)
reg_alpha_list = np.logspace(-1, 0, 3)
length_scale_list = np.logspace(-0.2, 1, 8)
...
param_grid = {'lam': lambda_list, 'alpha': reg_alpha_list, 'length_scale': length_scale_list}
...
def evaluate_params(lam, alpha, ls, Xj_train, ret_all, rf_all, ext_features_df, jm_setting, tr_cost):
    ...
    for train_idx, val_idx in tscv.split(Xj_train):
        ...
        # JumpModel のフィッティング
        jm = JumpModel(**jm_setting,sp_kernel_params = {'alpha': 1, 'length_scale':ls}, normalize_laplacian = True, jump_penalty=lam, n_kernel_features=min(500, len(train_idx)//2))
        ...
        cv_scores.append(mu)
    return lam, alpha, ls, np.mean(cv_scores)
    ...
    while t0 + relativedelta(months=pred_months) <= val_end:
        ...
        results = Parallel(n_jobs=-1, verbose=10)(
            delayed(evaluate_params)(
                params['lam'], params['alpha'],params['length_scale'],
        ...
        best_lambda, best_alpha, best_length_scale, best_score = max(results, key=lambda x: x[3])
        print(f"Best: jump penalty ={best_lambda}, reg_alpha ={best_alpha}, length_scale={best_length_scale}, score={best_score:.3f}")

        n_samples = Xj_train.shape[0]
        n_kernel_features = min(1000, n_samples // 2)
        jm_final = JumpModel(**jm_setting,sp_kernel_params = {'alpha': 1, 'length_scale':best_length_scale}, normalize_laplacian = True,jump_penalty=best_lambda,n_kernel_features=n_kernel_features)
...
import pickle
resdir = 'results'
nbname = 'UST10_cont1.4.5_GPU_rqpen_RKHSd_alpha1_v2'
