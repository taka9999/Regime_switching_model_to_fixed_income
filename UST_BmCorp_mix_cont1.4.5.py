...
for hl in hl_ext:
    vol2[f"vol2_exm_{hl}"] = vol_df2["hsvol"].ewm(halflife=hl).mean()
...
jm_setting = dict(
    ...
    #jump_penalty=jump_penalty,#set in each application
    cont=True,
    #kernel = 'rq',kernel_params = {'length_scale' : 100, 'alpha' : 20},#n_kernel_features = 1000, # set in each application
...
for pname, (umodel, ext_features_df) in patterns.items():
    ...
    while t0 + relativedelta(months=pred_months) <= val_end:
        ...
        X_feat_full = X_feat_full.dropna(axis=0, how='all')

        xgb_final = XGBClassifier(objective = "binary:logistic", eval_metric='logloss', random_state = 42, reg_alpha=best_alpha)
        xgb_final.fit(X_feat_full.values, y_train_full.values)
        importances = pd.Series(xgb_final.feature_importances_, index=X_feat_full.columns)
        ...
        p_xgb = xgb_final.predict_proba(X_feat_val.values)
        smooth = pd.Series(p_xgb[:, 1], index=X_feat_val.index).ewm(halflife=4, adjust=False).mean()
        y_pred = (smooth > 0.5).astype(int)
...
import pickle
resdir = 'results'
nbname = 'UST_BmCorp_mix_cont1.4.5_GPU_addfeat'
