...
for hl in hl_ext:
    bi[f"bi5_diff_ewma_{hl}"] = bi5_raw["PX_LAST"].diff().ewm(halflife=hl).mean()
    bi[f"bi10_diff_ewma_{hl}"] = bi10_raw["PX_LAST"].diff().ewm(halflife=hl).mean()
...
for hl in hl_ext:
    move[f"move_logdiff_ewma_{hl}"] = np.log(move_raw["PX_LAST"]).diff().ewm(halflife=hl).mean()
...
for hl in hl_ext:
    vol[f"volatility_{hl}"] = vol_df["volatility"].ewm(halflife=hl).mean()
...
for hl in hl_ext:
    spvol[f"spvol_{hl}"] = spvol_df["hsvol"].ewm(halflife=hl).mean()
...
for hl in hl_ext:
    corr[f"corr_diff_ewma_{hl}"] = corr_df["corr"].diff().ewm(halflife=hl).mean()
...
for hl in hl_ext:
    sw[f"sw2_diff_ewma_{hl}"] = sw02_raw["PX_LAST"].diff().ewm(halflife=hl).mean()
    sw[f"sw10_diff_ewma_{hl}"] = sw10_raw["PX_LAST"].diff().ewm(halflife=hl).mean()
    sw[f"sw3m_diff_ewma_{hl}"] = sw3m_raw["PX_LAST"].diff().ewm(halflife=hl).mean()
    sw[f"sw_slope_ewma_{hl}"] = (sw10_raw["PX_LAST"] - sw02_raw["PX_LAST"]).diff().ewm(halflife=hl).mean()
...
import pickle
resdir = 'results'
nbname = 'UST10_cont1.4.5_GPU_rqKPLS_addfeature'
