...
## Data Preparation
df = pd.read_csv(datadict + assetfile + '.csv',index_col = 0,skiprows = 5)
df = df.reset_index()
...
vol_df2 = pd.read_csv(volfile2, index_col=0)
vol_df2 = vol_df2.reset_index()
vol_df2 = vol_df2.sort_values("date", ascending=True)
vol_df2["date"] = pd.to_datetime(vol_df2["date"])
vol_df2.set_index("date",inplace = True)
vol2 = pd.DataFrame(index = vol_df2.index)
vol2["vol2_diff_ewma"] = vol_df2["hsvol"].diff().ewm(halflife=5).mean()
vol2_aligned = vol2.reindex(ext_features_pre.index, method="ffill")
...
spvol2_df = pd.read_csv(spvolfile2, index_col=0)
spvol2_df = spvol2_df.reset_index()
spvol2_df = spvol2_df.sort_values("date", ascending=True)
spvol2_df["date"] = pd.to_datetime(spvol2_df["date"])
spvol2_df.set_index("date", inplace=True)
spvol2 = pd.DataFrame(index=spvol2_df.index)
spvol2["spvol_diff_ewma"] = spvol2_df["volatility"].diff().ewm(halflife=5).mean()
spvol2_aligned = spvol2.reindex(ext_features_pre.index, method="ffill")
...
corr2_df =pd.read_csv(corrfile2, index_col=0)
corr2_df = corr2_df.reset_index()
corr2_df = corr2_df.sort_values("date", ascending=True)
corr2_df["date"] = pd.to_datetime(corr2_df["date"])
corr2_df.set_index("date", inplace=True)
corr2 = pd.DataFrame(index=corr2_df.index)
for hl in hl_ext:
    corr2[f"corr2_diff_ewma_{hl}"] = corr2_df["corr"].diff().ewm(halflife=hl).mean()
corr2_aligned = corr2.reindex(ext_features_pre.index, method="ffill")
...
fred_df = pd.read_pickle('data/fred_df.pkl')
y2   = fred_df[["DGS2"]].rename(columns={"DGS2": "y2"})
y10  = fred_df[["DGS10"]].rename(columns={"DGS10": "y10"})
swsp =pd.DataFrame(index=sw10_raw.index)
swsp["s2"] = sw02_raw["PX_LAST"]
swsp["s10"] = sw10_raw["PX_LAST"]
swsp = pd.concat([y2,y10,swsp],axis = 1, join ='inner').reindex(ext_features_pre.index, method="ffill")
swsp['swsp2_diff_ewma'] = (swsp['s2'] - swsp['y2']).diff().ewm(halflife=5).mean()
swsp['swsp10_diff_ewma'] = (swsp['s10'] - swsp['y10']).diff().ewm(halflife=5).mean()
swsp = swsp[["swsp2_diff_ewma", "swsp10_diff_ewma"]]

ext_features_pre = pd.concat([ext_features_pre,
                        bi_aligned, 
                       move_aligned, 
                       vol_aligned,
                       vol2_aligned,
...
lambda_list = np.logspace(4,5,4)
reg_alpha_list = np.logspace(-1, 0, 4)
n_comp_list = [2 , 6, 10, 14, 18]
#alpha = 0.1
#best_alpha = 0.1
...
# パラメータの全組み合わせを作成
param_grid = {'lam': lambda_list, 'alpha': reg_alpha_list, 'n_comp': n_comp_list}
grid = list(ParameterGrid(param_grid))

# クロスバリデーションを回して平均スコアを返す関数
def evaluate_params(lam,alpha, n_comp, Xj_train, ret_all, rf_all, ext_features_df, jm_setting, tr_cost):
    ...
    for train_idx, val_idx in tscv.split(Xj_train):
        ...
        y_tr_bin   = y_train.values                       # (n_samples,)
        
        rq = RationalQuadratic(length_scale=5, alpha=5.0)
        #K_tr_lin = linear_kernel(X_tr_mat, X_tr_mat)
        K_tr_rq  = rq(X_tr_mat, X_tr_mat)
        K_tr      = K_tr_rq


        #gamma = 1.0 / X_tr_mat.shape[1]      # RBF の幅は 1/#features が目安
        #K_tr = rbf_kernel(X_tr_mat, X_tr_mat, gamma=gamma)  # (n_tr, n_tr)

        pls = PLSRegression(n_components=n_comp)
        X_tr_lat,_   = pls.fit_transform(K_tr, y_tr_bin)    # → 確認: shape が (n, best_nc) の 2D

        # 3) ロジスティック回帰で学習
        xgb = XGBClassifier(objective='binary:logistic',eval_metric='logloss',random_state=42,reg_alpha = alpha,base_score = 0.5)
        #xgb.fit(X_feat_tr.values, y_train.values)
        xgb.fit(X_tr_lat, y_tr_bin)                           # → 通れば次へ
        #clf = LogisticRegression(solver="lbfgs")
        #clf.fit(X_tr_lat, y_tr_bin)                           # → 通れば次へ

        # —— 予測 —— 
        # 4) 検証データも同じ処理を踏む
        X_val_mat = scaler.transform(X_feat_val.values)
        K_val_lin = linear_kernel(X_val_mat, X_tr_mat)
        K_val_rq  = rq(X_val_mat,    X_tr_mat)
        #K_val = rbf_kernel(X_val_mat, X_tr_mat, gamma=gamma) # (n_val, n_tr)
        K_val = K_val_rq
        
        X_val_lat = pls.transform(K_val)                   # (n_val, n_comp)

        # 5) 確率予測
        #y_val_pred = clf.predict_proba(X_val_lat)[:,1] # 1列目はクラス0の確率なので、1列目を取る
        y_val_pred = xgb.predict_proba(X_val_lat)[:,1] # 1列目はクラス0の確率なので、1列目を取る
        ...
        cv_scores.append(mu)
    return lam, alpha, n_comp, np.mean(cv_scores)
    ...
    while t0 + relativedelta(months=pred_months) <= val_end:
        ...
        results = Parallel(n_jobs=-1, verbose=10)(
            delayed(evaluate_params)(
                params['lam'], params['alpha'], params['n_comp'],
        ...
        best_lambda, best_alpha, best_n_comp, best_score = max(results, key=lambda x: x[3])
        print(f"Best: jump penalty ={best_lambda}, reg_alpha = {best_alpha}, n_comp ={best_n_comp}, score={best_score:.3f}")
        ...
        scaler_full = StandardScalerPD().fit(X_feat_full.values)
        X_tr_mat_full = scaler_full.transform(X_feat_full.values)
        
        rq = RationalQuadratic(length_scale=5, alpha=5.0)
        #K_tr_lin = linear_kernel(X_tr_mat_full, X_tr_mat_full)
        K_tr_rq  = rq(X_tr_mat_full, X_tr_mat_full)
    
        K_tr = K_tr_rq
        
        #gamma = 1.0 / X_tr_mat_full.shape[1]      # RBF の幅は 1/#features が目安
        #K_tr = rbf_kernel(X_tr_mat_full, X_tr_mat_full, gamma=gamma)  # (n_tr, n_tr)
    
        pls_full = PLSRegression(n_components=best_n_comp).fit(K_tr, y_train_full.values)
    
        X_tr_lat_full = pls_full.transform(K_tr)  # (n_samples, best_nc)
    
        xgb_full = XGBClassifier(objective='binary:logistic',eval_metric='logloss',random_state=42, reg_alpha = best_alpha, base_score = 0.5)
        #xgb_full.fit(X_feat_full.values, y_train_full.values)
        xgb_full.fit(X_tr_lat_full, (y_train_full>0.5).astype(int))
        #clf_full = LogisticRegression(solver="lbfgs").fit(X_tr_lat_full, (y_train_full>0.5).astype(int))
        ...
        #X_feat_val_full = X_feat_val_full.dropna(axis=1, how='all')
        ...
        # 確率を取り出す
        y_val_prob_full = xgb_full.predict_proba(X_val_lat_full)[:,1]
        #y_val_prob_full = clf_full.predict_proba(X_val_lat_full)[:,1]
    
        pred_idx = X_feat_val_full.index[1:]
        #p_xgb = xgb_full.predict_proba(X_feat_val_full.values)
        #smoothed = pd.Series(p_xgb[:, 1], index=X_feat_val_full.index).ewm(halflife=4, adjust=False).mean()
        smoothed  = pd.Series(y_val_prob_full, index=X_feat_val_full.index).ewm(halflife=4, adjust=False).mean()
...
import pickle
resdir = 'results'
nbname = 'BmCorp_cont1.4.5_GPU_corr2_KPLS_rqpen'
