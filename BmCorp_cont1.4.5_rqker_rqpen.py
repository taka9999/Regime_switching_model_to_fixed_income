...
jm_setting = dict(
    ...
    #jump_penalty=jump_penalty,#set in each application
    cont=True,
    kernel = 'rq',kernel_params = {'alpha': 20, 'length_scale':5},#n_kernel_features = 1000, # set in each application
...
lambda_list = np.logspace(2,3.5,3)
...
def evaluate_params(lam, alpha, Xj_train, ret_all, rf_all, ext_features_df, jm_setting, tr_cost):
    ...
    for train_idx, val_idx in tscv.split(Xj_train):
        ...
        smooth = pd.Series(p_tr[:, 1], index = X_feat_val.index).ewm(halflife = 8, adjust = False).mean()
...
for pname, (umodel, ext_features_df) in patterns.items():
    ...
    while t0 + relativedelta(months=pred_months) <= val_end:
        ...
        #pls_full = PLSRegression(n_components=best_nc).fit(K_tr, y_train_full.values)
        ...
        smoothed = pd.Series(p_xgb[:, 1], index=X_feat_val_full.index).ewm(halflife=8, adjust=False).mean()
...
import pickle
resdir = 'results'
nbname = 'BmCorp_cont1.4.5_GPU_corr2_rqker_rqpen'
