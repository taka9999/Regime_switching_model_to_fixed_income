...
#lambda_kl_list = np.concatenate(([0], np.logspace(-4, -2, 3)))
lambda_kl_list = np.logspace(-2, 0, 3)

param_grid = {'lam': lambda_list, 'lam_kl': lambda_kl_list}
...
class SeqRegimeDataset(Dataset):
    ...
    def __getitem__(self, i):
        # i=0 のとき samples [0:L-1] を取り，ラベルは y[L]
        seq = self.X[i : i + self.L]        # shape (L, D)
        label = self.y[i + self.L]          # 次の時刻の regime label
        return torch.from_numpy(seq), torch.tensor(label,dtype = torch.float32)


class RegimeRNN(nn.Module):
    def __init__(self, n_features, hidden_sizes, dropout=0.3):
        super().__init__()
        layers = []
        in_size = n_features
        ...
        self.post_ln = nn.LayerNorm(hidden_sizes[-1])
        self.reggressor = nn.Linear(hidden_sizes[-1], 1)

    def forward(self, x):
        # x: (batch, seq_len, n_features)
        x = x.transpose(1, 2)               # -> (batch, n_features, seq_len)
        x = self.input_bn(x)               # feature-wise 正規化
        x = x.transpose(1, 2)               # -> (batch, seq_len, n_features)
        
        out, _ = self.lstm(x)
        hn = self.post_ln(out)
        logits_seq = self.reggressor(hn).squeeze(-1)
        logits_last = logits_seq[:,-1]
        return logits_last, logits_seq
L = 20
...
def bernoulli_kl_from_logits(a, b):
    # a=logit(p_t), b=logit(p_{t-1})
    p = torch.sigmoid(a)
    # log σ(x) = -softplus(-x), log σ(-x) = -softplus(x)
    return p * (-torch.nn.functional.softplus(-a) + torch.nn.functional.softplus(-b)) + (1-p) * (-torch.nn.functional.softplus(a)  + torch.nn.functional.softplus(b))

# クロスバリデーションを回して平均スコアを返す関数
def evaluate_params(lam, lam_kl, Xj_train, ret_all, rf_all, ext_features_df, jm_setting, tr_cost):
    ...
    for train_idx, val_idx in tscv.split(Xj_train):
        ...
        X_tr_np = scaler.fit_transform(X_feat_tr.values).astype(np.float32)
        X_val_np = scaler.transform (X_feat_val.values).astype(np.float32)
        y_tr_np = y_train.values.astype(np.float32)
        # テストラベルはval_loader用
        labels_test_full = pd.Series(
            np.zeros(len(X_feat_val), dtype=np.float32),
            index=X_val_fold.index
        )

        # 3) Dataset/DataLoader
        train_ds = SeqRegimeDataset(X_tr_np, y_tr_np, L)
        ...
        for epoch in range(n_epochs):
            ...
            for Xb, yb in train_loader:
                #assert not torch.isnan(Xb).any(), "X contains NaN!"
                #assert not torch.isinf(Xb).any(), "X contains Inf!"
                #assert not torch.isnan(yb).any(), "y contains NaN!"
                #break
                Xb = Xb.to(device)
                yb = yb = yb.unsqueeze(1).to(device)
                optimizer.zero_grad()
                logits_last, logits_seq = model(Xb)
                loss_main   = criterion(logits_last, yb.squeeze(1))
                #tv_l2 = (logits_seq[:,1:] - logits_seq[:,:-1]).pow(2).mean()
                u = logits_seq[:, 1:] - logits_seq[:, :-1]  # (B, L-1)
                tv_l1 = (u.abs() + 1e-6).mean()            # 近傍εで滑らかに
                delta = 0.1
                tv_huber = torch.where(u.abs() <= delta,
                                    0.5 * u.pow(2),
                                    delta * (u.abs() - 0.5 * delta)).mean()
                tv_l2 = u.pow(2).mean()
                u2 = logits_seq[:, 2:] - 2*logits_seq[:, 1:-1] + logits_seq[:, :-2]
                curv_l2 = u2.pow(2).mean()
                kl_t = bernoulli_kl_from_logits(logits_seq[:,1:], logits_seq[:,:-1]).mean()
                loss  = loss_main + lam_kl * kl_t
                #loss = loss_main + lambda_tv * tv_l1
                loss.backward()
                optimizer.step()
                total_loss += loss.item() * yb.size(0)
            #print(f"{pname} Epoch {epoch+1}/{n_epochs}, CV_term_training_loss={total_loss/len(train_ds):.4f}")
        
        X_ctx = np.concatenate([X_tr_np[-L:], X_val_np], axis=0)
        # 6) テスト評価ループ
        model.eval()
        all_preds = []
        with torch.no_grad():
            for t in range(len(X_val_np)):
                seq = X_ctx[t : t + L]                       # shape: (L, n_features)
                x = torch.from_numpy(seq[None]).to(device)   # shape: (1, L, n_features)
                logits, _ = model(x)                          # BCEWithLogits想定 -> (1, 1)
        ...
        p_tr = pd.Series(all_preds, index=val_idx)
        smooth = pd.Series(p_tr, index = X_feat_val.index)#.ewm(halflife = 4, adjust = False).mean()
        ...
        cv_scores.append(mu)
    return lam, lam_kl, np.mean(cv_scores)
...
for pname, (umodel, ext_features_df) in patterns.items():
    ...
    while t0 + relativedelta(months=pred_months) <= val_end:
        ...
        best_lambda, best_lam_kl, best_score = max(results, key=lambda x: x[2])
        print(f"Best: jump penalty ={best_lambda}, lam_kl ={best_lam_kl}, score={best_score:.3f}")
        ...
        scaler = StandardScaler()
        X_tr_np = scaler.fit_transform(X_feat_full.values).astype(np.float32)
        y_tr_np = y_train_full.values.astype(np.float32)
        ...
        for epoch in range(n_epochs):
            ...
            for Xb, yb in train_loader:
                ...
                loss_main   = criterion(logits_last, yb.squeeze(1))
                #tv_l2 = (logits_seq[:,1:] - logits_seq[:,:-1]).pow(2).mean()
                #loss = loss_main + lambda_tv * tv_l2
                u = logits_seq[:, 1:] - logits_seq[:, :-1]  # (B, L-1)
                ...
                #loss = loss_main + lambda_tv * tv_l1
                kl_t = bernoulli_kl_from_logits(logits_seq[:,1:], logits_seq[:,:-1]).mean()
                loss  = loss_main + best_lam_kl * kl_t
                loss.backward()
                optimizer.step()
                total_loss += loss.item() * yb.size(0)
            #print(f"{pname} Epoch {epoch+1}/{n_epochs}, Final_fit_loss={total_loss/len(train_ds):.4f}")
        ...
        X_feat_val_np = scaler.transform(X_feat_val_full.values).astype(np.float32)
        labels_jm_full = pd.Series(
                        jm_true,
                        index=Xj_pred.index
                    )
        
        val_ds  = SeqRegimeDataset(X_feat_val_np, labels_jm_full.values.astype(np.int64), L)
        val_loader  = DataLoader(val_ds,  batch_size=128, shuffle=False, num_workers=0)
        
        X_ctx = np.concatenate([X_tr_np[-L:], X_feat_val_np], axis=0)
        ...
        with torch.no_grad():
            for t in range(len(X_feat_val_np)):
                seq = X_ctx[t : t + L]                       # shape: (L, n_features)
                x = torch.from_numpy(seq[None]).to(device)   # shape: (1, L, n_features)
                logits, _ = model(x)                          # BCEWithLogits想定 -> (1, 1)
                preds  = torch.sigmoid(logits)
        ...
        smoothed  = pd.Series(all_preds, index=X_feat_val_full.index)#.ewm(halflife=4, adjust=False).mean()
...
import pickle
resdir = 'results'
nbname = 'UST10_cont1.4.5_GPU_rqpen_regLSTM'
