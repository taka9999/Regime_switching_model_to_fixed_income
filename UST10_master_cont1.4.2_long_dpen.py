...
lambda_list = np.logspace(3.5, 7.5 ,6)
    ...
    while t0 + relativedelta(months=pred_months) <= val_end:
        ...
        for lam in lambda_list:
            ...
            for train_idx, val_idx in tscv.split(Xj_train):
                ...
                p_tr = jm.predict_online(X_val_fold.values)
        ...
        best_model = JumpModel(**jm_setting, jump_penalty=best_lambda, n_kernel_features = 1000)
        best_model.fit(Xj_train, ret_all, sort_by = "cumret")

        Xj_pred = X_all.loc[pred_start:pred_end]
        pred_idx  = Xj_pred.index[1:]
        p_jmo = best_model.predict_online(Xj_pred.values)
        smooth = pd.Series(p_jmo, index=Xj_pred.index)#.ewm(halflife=4, adjust=False).mean()
        y_pred = (smooth > 0.5).astype(int)
...
import pickle
resdir = 'results'
nbname = 'UST10_master_cont1.4.2_long_dpen'
